...
// @downloadURL  https://github.com/Audionut/add-trackers/raw/main/ptp-add-filter-all-releases-anut.user.js
// @updateURL    https://github.com/Audionut/add-trackers/raw/main/ptp-add-filter-all-releases-anut.user.js
...
(function () {
  ...
  const fields = {
    ...
    show_name: {
      ...
      type: 'checkbox',
      default: true,
      tooltip: 'Display the tracker name next to releases'
    },
    hide_filters: {
    ...
    filterboxlocation: {
      ...
      type: 'select',
      options: ['Above', 'Below', 'Torrents'],
      default: 'Below',
      tooltip:
        'Choose where to display the filter box. Above places above the movie poster, below places below. Torrents places it above the torrent group OG style.'
    },
    simplediscounts: {
    ...
    tracker_by_default: {
      ...
      type: 'text',
      default: '',
      tooltip: 'Show only these sites by default. Comma separated. PTP, BHD, ANT, etc'
    },
    res_by_default: {
    ...
    hideBlankLinks: {
      label: 'How to display download link',
      type: 'select',
      options: ['DL', 'Download', 'Spaced'],
      default: 'DL',
      tooltip:
        'Choose how to display the download links: DL (original method), DOWNLOAD, or Spaced. Other methods help with some stylesheets.'
    },
    timer: {
  ...
  if (globalThis.location.href.includes('/torrents.php?id=')) {
    ...
    const isMiniSeriesFromSpan = Array.from(
      document.querySelectorAll('span.basic-movie-list__torrent-edition__main')
    ...
    const listItems = detailsCollections
      ? detailsCollections.querySelectorAll('ul.list--unstyled li')
      : null;
    const isMiniSeriesFromList = listItems
              ...
              if (!badGroupFound) {
                ...
                if (matchedGroup) {
                  ...
                  if (match) {
                    ...
                    groupText = groupText.replaceAll(/[^a-z0-9]/ig, '');
                    if (improved_tags) {
    ...
    let yearMatch = /\[\d{4}\]/.exec(name_url);
    let pageYear = yearMatch ? Number.parseInt(yearMatch[0].replaceAll(/[[\]]/g, '')) : null;
          ...
          const results = await Promise.all(
            postData.results.map(async (d) => {
              ...
              if (/S\d{1,2}E\d{1,2}/.test(infoText) === false) {
                ...
                const match = infoText.match(/(?:-(?!\.))([a-zA-Z][a-zA-Z0-9]*)$/);
                if (match) {
                  groupText = match[1].replaceAll(/[^a-z0-9]/ig, '');
                  if (improved_tags) {
                    infoText = infoText.replace(`-${match[1]}`, '').trim();
                  }
                }
      ...
      if (
        ...
        torrent_objs = json.data
          .map((element) => {
            ...
            if (!badGroupFound) {
              ...
              if (matchedGroup) {
                ...
                if (match) {
                  ...
                  groupText = groupText.replaceAll(/[^a-z0-9]/ig, ''); // Sanitize to alphanumeric characters
                  if (improved_tags) {
            ...
            const imageUrls = [];
            const bbCodeImageRegex =
              /\[img(?:=\d+)?\](https?:\/\/[^\s[\]]+?\.png)\[\/img\]/gi;
        ...
        if (improved_tags && show_tracker_name) {
          if (tracker === 'RED' || tracker === 'OPS') {
            ...
            cln.querySelector('.torrent-info-link').textContent =
              `[${torrent.site}] / ` + get_simplified_title(torrent.info_text);
          }
          ...
          cln.querySelector('.torrent-info-link').textContent =
            `[${torrent.site}] ` + torrent.info_text;
        } else {
    ...
    const fix_ptp_names = () => {
      document.querySelectorAll('tr.group_torrent').forEach((d) => {
        ...
        } else if (d.className !== 'group_torrent') {
          const torrent = d.querySelector('a.torrent-info-link');
          if (torrent) {
            if (improved_tags && show_tracker_name) {
    ...
    const add_filters_div = (trackers, discounts, qualities) => {
      ...
      let addBeforeThis = document.querySelector(
        filterboxlocationMap[filterboxlocation] || '#movieinfo'
      );
    ...
    const mainFunc = async () => {
      ...
      try {
        ...
        imdb_id =
          'tt' + document.getElementById('imdb-title-link').href.split('/tt')[1].split('/')[0];
      } catch (e) {
        console.log(
          'Failed to extract IMDb ID using imdb-title-link, attempting fallback method:',
          e
        );
      ...
      let year = null;
        const yearMatch = /\[(\d{4})\]/.exec(name_url); // Matches a year in the format [YYYY]
      if (yearMatch) {
      ...
      Promise.all(promises).then((torrents_lists) => {
        // Combine all torrents into one array
          const all_torrents = torrents_lists.flat().sort((a, b) => {
    ...
    const addLink = function () {
      const menu = document.querySelector('#content > div > div.linkbox');
      if (menu) {
