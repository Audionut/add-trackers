...
function toggleRowGroup(startRow, hide) {
  ...
  function applySettings() {
    const rowGroups = document.querySelectorAll('.basic-movie-list__torrent-edition__sub');
    const applyForMultipleGroupsOnly = GM_config.get('applyForMultipleGroupsOnly');
  
    if (!applyForMultipleGroupsOnly || rowGroups.length > 1) {
      rowGroups.forEach(rowGroup => {
  ...
  function createSettingsMenu() {
    const rowGroups = document.querySelectorAll('.basic-movie-list__torrent-edition__sub');
    const fields = {
      applyForMultipleGroupsOnly: {
        label: "Don't hide group if it's the only group",
        type: 'checkbox',
        default: GM_getValue('applyForMultipleGroupsOnly', true)
      }
    };
        ...
        save: function () {
          ...
          for (const field in fields) {
            if (fields.hasOwnProperty(field)) {
              const isChecked = GM_config.get(field);
              GM_setValue(field, isChecked);
            }
          }
